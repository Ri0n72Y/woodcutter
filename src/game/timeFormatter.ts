...
export function convertDays(d: IDate, offset = 0): string {
    const d1 = (d.date % DAY_EACH_YEAR + offset) % 10;
    const d10 = Math.floor((d.date % DAY_EACH_YEAR + offset) / 10);
    return `${time[d.year]}年${d10 > 0 ? tens[d10 - 1] : ''}${time[d1]}日 - ${season[d.season]}`;
}
